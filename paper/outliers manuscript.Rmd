...
#fisher.test(reason.table.test, simulate.p.value=TRUE,B=1e7)
...
long.test=master[,c("time.pulled", "Type", "mention.outliers", "peopleor.data.points", "take.out.or.leave.in", "run.with.or.without", 
                             "Basics", "ANOVA" , "Regression", "ChiSquare", "Nonparametric", 
                            "Modeling", "BayesOther" )]

long.test.data=melt(long.test, 
                    id = c("time.pulled", "Type","mention.outliers", "peopleor.data.points", "take.out.or.leave.in", "run.with.or.without"),
                    measured = c("Basics", "ANOVA" , "Regression", "ChiSquare", "Nonparametric", 
                                 "Modeling", "BayesOther"))
#View(choice.data)          
colnames(long.test.data)=c("time.pulled", "Type","mention.outliers", "peopleor.data.points", "take.out.or.leave.in", "run.with.or.without", 
                            "analysis", "used")
View(long.test.data)
...
# Results
Investigating reporting of outliers, we found that in 2012, X% of articles--X% of experiments--mentioned outliers, while in 2017 X% of articles--X% of experiments--mentioned outliers. This represents a significant increase in mentions of outliers *chi^2* (`r chisq.test(table1)$Table[1, 2]`) = `r chisq.test(table1)$Table[1,1]`, *p* = `r chisq.test(table1)$Table[1,3]`. Further exploration reveals that differences? in reporting between years arise between fields (*chi^2* (`r model2$Table[3, 3]`) = `r model2$Table[3,2]`, *p* = `r model2$Table[3,3]`), which can be seen in \@ref(tab:journalprop). Specifically, if we look at the change in reporting for each field analyzed at the level of the experiment  (HEYO, CAN I GET A TABLE IN HERE LIKE ON THE POSTER WITH THE EFFECT SIZES?) we find the largest differences appear for Social, Cognitive, I/O, and Forensic Psychology, with the smallest changes appearing for the Environmental, Overview, and Clinical Psychology. 
When viewing the reporting of outliers broken down by analysis across time, we see there is a significant association between time, analysis, and outlier reporting (insert stats here). Every analysis shows an increase in mentions of outliers over time, and when looking solely the relationship between analysis and time of those experiments actually reporting outliers, we see the largest increases in reporting for the Baysian/Other category, followed by the modeling category, and the nonparametric category.  
...
In our review, the majority of outliers mentioned referred to people (x%), removed the observation (x%), and ran the analyses without the observation (x%). Only 10 experiments (x%) discussed the differences between results both with and without the outlying ovservations, and of those, 6 found no differences in their analyses and 4 found differences in their analyses. Of the 6 that found no differences, they all reported their final analyses without the outliers. Of the 4 that found differences, 1 left the outliers in, and 2 reported the results both with and without the outliers. (CAN WE PUT A TABLE FOR THIS PARAGRAPH THAT HAS A ROW FOR EACH OUTCOME, I.E. WITH/WITHOUT/WINSORIZED, AND A COLUMN FOR THE %(N) FALLING INTO THAT CATEGORY?)
