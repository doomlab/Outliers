...
library(dplyr)
df<-master %>% 
    mutate(id = group_indices_(master, .dots=c("article"))) 

m1 <- glmer(mention.outliers2 ~ year  + (1 | Type),
            data=df, 
...
m1 <- glmer(mention.outliers2 ~ year + Type  + (1 | Journal),
            data=df, 
            family=binomial(link="logit"),
            control = glmerControl(optimizer = "bobyqa"),
            nAGQ = 0)
...
m2 <- glmer(peopleor.data.points.bi ~ year  + (1 | Type),
