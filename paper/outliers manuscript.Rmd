...
#we are increasing in sample size over time, slightly, rho=.137, p<.001
plot(master$year, master$original.sample.size) #wtf is up with year here?

```

# Results
Investigating reporting of outliers, we found that in 2012, X% of articles--X% of experiments--mentioned outliers, while in 2017 X% of articles--X% of experiments--mentioned outliers. This represents a significant increase in mentions of outliers, PUT CHISQ STATS HERE. Further exploration reveals that differences in reporting between years arise between fields (INSERT CHSQ HERE) which can be seen in \@ref(tab:journalprop). Specifically, if we look at the change in reporting for each field analyzed at the level of the experiment (HEYO, CAN I GET A TABLE IN HERE LIKE ON THE POSTER WITH THE EFFECT SIZES?) we find the largest differences appear for Social, Cognitive, I/O, and Forensic Psychology, with the smallest changes appearing for the Environmental, Overview, and Clinical Psychology. 
When viewing the reporting of outliers broken down by analysis across time, we see there is a significant association between time, analysis, and outlier reporting (insert stats here). Every analysis shows an increase in mentions of outliers over time, and when looking solely the relationship between analysis and time of those experiments actually reporting outliers, we see the largest increases in reporting for the Bayesian/Other category, followed by the modeling category, and the nonparametric category.  
