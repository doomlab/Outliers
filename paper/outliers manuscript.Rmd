...
?apa_print
# Results
Investigating reporting of outliers, we found that in 2012, X% of articles--X% of experiments--mentioned outliers, while in 2017 X% of articles--X% of experiments--mentioned outliers. This represents a significant increase in mentions of outliers, $\chi^2$ (`r stuff$parameter`) = `r stuff$statistics`, *p*< .001 PUT CHISQ STATS HERE. Further exploration reveals that differences in reporting between years arise between fields (INSERT CHSQ HERE) which can be seen in \@ref(tab:journalprop). Specifically, if we look at the change in reporting for each field analyzed at the level of the experiment (HEYO, CAN I GET A TABLE IN HERE LIKE ON THE POSTER WITH THE EFFECT SIZES?) we find the largest differences appear for Social, Cognitive, I/O, and Forensic Psychology, with the smallest changes appearing for the Environmental, Overview, and Clinical Psychology. 
