...
Types of analyses were broadly defined as basic statistics (descriptive statistics, *z*-scores, *t*-tests, and correlations), ANOVAs, regressions, chi-squares, nonparametric statistics, modeling, and Bayesian/other analyses. 
...
```{r info-journal, include=FALSE}
tableprint = matrix(NA, nrow = 31, ncol = 6)
colnames(tableprint) = c("Field", "Journal", "Publisher", "Impact2012", 
                         "Impact2017", "5Year")
...
write.csv(tableprint, "Journal Information.csv", row.names = F)
```
...
# Results
Data processing resulted in a total of 2234 experiments being coded, 1085 of which were from 2012 or prior, and the additional 1149 being from 2017 or prior. Investigating reporting of outliers, we found that in 2012, `r overalloutpercent$percent[2]`% of experiments mentioned outliers, while in 2017 `r overalloutpercent$percent[4]`% of experiments mentioned outliers. When evaluted withhin the context of a multilevel model using the year the experiment was pusblished to predict whether or not outliers were mentioned and inclduing a random intercept for journal we find that year significantly predicts mentioning outliers, *b*= `r m1$beta[2]`, SE=0.02, *p* <.001. Each year, experiemtns were 13.5% more likely to report outliers as the previous year. Further exploration reveals that differences in reporting between years arise between fields  which can be seen in \@ref(tab:journalprop). Specifically, if we look at the change in reporting for each field analyzed at the level of the experiment we find the largest changes in forensic (45% more likely to report), social (34%), and IO (33%). Following these are developmental (20%) and cognitive (15%). These analyses show that in some fields, including our overall category and neuro psychology  an insignificant but intersting suggestion that reporting of outliers may actually be decreasing slightly. Additionally, we find that the variability due to journal fis consistently small throughout all fields.

We then investigated how this reporting between years may differ by analysis, as can be seen in \@ref(????) (CAN I GET A TABLE HERE WITH A ROW FOR EACH ANALYSIS TYPE AND A COLUMN FOR N IN 2012, N IN 2017, ODD RATIO INCREASE, AND JOURNAL VARIANCE PLEASE). Increase in reporting was significant for nonparamertic (38% more likely to report), basic (23%), regression (15%), anova (14%), and modeling (11%) techniques. Additionally, we find some variability due to journal between analyses. 

Of those outliers that were reported, the majority of the time this was reportedly due to statistical reasons (47.7%) and/or participant errors (47.1%), with few experiments reporting outliers due to unusable data (5.2%). HOW CAN WE ANALYZE THIS NOW THAT WE HAVE YEAR? LIKE A MULTINOMIAL LOGIT? BUT THEY AREN'T UNIQUE CATEGORIES..? When looking at the association of these reasons over time, we again clearly see all reasons increasing in usage from 2012 to 2017, but see the most increase in reporting of participant errors (3.5 times more often reported in 2017), followed by unusable data (3.3 times), and finally statistical reasons (1.7 times). 

In our review, the majority of outliers mentioned referred to people (65.9%) as opposed to data points (25.3%), or both people and data points (5.7%), a final 3% of experiments mentioned outliers but did not specify a type, just that they found none. We were addtiiaonlly interested investigating the differene in reporting of just people or data points (dropping the both or none categories) across time. Using a multilevel model predicting whether people or data points were removed using year and a random intercept for journal we find no significant effect for year, b=0.05, SE=0.05, *p*=.36, demondtrating that the change in reporting of both types of outliers were consistent. When handling these data, some experiemnts chose to winzorize data (.7%), most analyzed the data without the observations (88.5%), some analyzed the data with the ovservations (7.4%), and some conducted analyses both with and without the observations (3.4%). Of the outliers found, the majority (88.1%) were removed from the dataset before analysis OK, NOW IM NOT SURE WHAT THIS IS REPORTING, CAN WE TALK AOBUT THE TAKE OUT IN VAR?. Only 9 experiments (2%) discussed the differences between results both with and without the outlying observations, and of those, 5 found no differences in their analyses and 4 found differences in their analyses. Of the 5 that found no differences, 3 reported their final analyses without the outliers, 1 reported their final analyses with the outlier included, and 1 reported the results both with and without the outliers. Of the 4 that found differences, 3 reported the results without the outliers and 1 reported the results both with and without the outliers.
